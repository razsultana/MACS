# Time-stamp: <2018-05-04 14:36:42 Tao Liu>
...
#                  maxgap = None, minlen = None, slocal = None, llocal = None):
        ...
        # if maxgap is 0, set maxgap as tag size
        if self.opt.maxgap == 0:
            self.maxgap = self.opt.tsize

        # if minlen is 0, set minlen as fragment size/d
        if self.opt.minlen == 0:
            self.minlen = self.opt.d
